...
```
复杂度分析:
时间复杂度：O(N)O(N)O(N)，其中 NNN 指的是数组 nums 的大小。预处理 L 和 R 数组以及最后的遍历计算都是 O(N)O(N)O(N) 的时间复杂度。
空间复杂度：O(N)O(N)O(N)，其中 NNN 指的是数组 nums 的大小。使用了 L 和 R 数组去构造答案，L 和 R 数组的长度为数组 nums 的大小。
...
复杂度分析
```
时间复杂度：O(N)O(N)O(N)，其中 NNN 指的是数组 nums 的大小。分析与方法一相同。
